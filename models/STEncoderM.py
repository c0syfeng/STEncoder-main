...
import numpy as np
from layers.Embed import DataEmbedding,DataEmbedding_inverted,Adaptive_Wavelet_Tranform
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.ch_mlps = nn.ModuleList([nn.Linear(self.repr_dims, 1) for _ in self.STEncoder_multiscales])
        ...
        self.enc_embedding_Season = DataEmbedding_inverted(configs.seq_len, configs.dd_model, configs.embed, configs.freq,
                                                    configs.dropout)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
                enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None, onlyrepr=False,use_norm=True):
        ...
        if self.STEncoder_wnorm == 'ReVIN':
            ...
            short_x_enc = (x_enc - seq_mean) / (seq_std + 1e-5)
            long_x = short_x_enc.clone()
        elif self.STEncoder_wnorm == 'Mean':
            seq_mean = x_enc.mean(dim=1, keepdim=True).detach()
            short_x_enc = (x_enc - seq_mean)
            long_x = short_x_enc.clone()
        elif self.STEncoder_wnorm == 'Decomp':
            #short_x, long_x = self.input_decom(x_enc)
            short_x_enc,long_x = self.input_decom(x_enc)
            short_x_dec, long_x_dec = self.input_decom(x_dec)
        elif self.STEncoder_wnorm == 'LastVal':
            seq_last = x_enc[:,-1:,:].detach()
            short_x_enc = (x_enc - seq_last)
            long_x = short_x_enc.clone()
        else:
        ...
        #enc_out = self.enc_embedding_t(long_x, x_mark_enc.squeeze(1).repeat(1, C, 1, 1).reshape(B*C, T, -1))  # B * C, T, D
        ...
        len_out = F.gelu(repr.transpose(1, 2))
        len_out = self.length_mlp(len_out).transpose(1, 2)  # (B * C, O, D)
