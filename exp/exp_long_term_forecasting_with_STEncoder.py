...
from models.STEncoderM import series_decomp
from layers.losses import STEncoderS, STEncoderM

warnings.filterwarnings('ignore')


class Exp_Long_Term_Forecast_with_STEncoder(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_with_STEncoder, self).__init__(args)

        self.STEncoder = args.STEncoder
        self.STEncoder_lambda = args.STEncoder_lambda
        st = time.time()
    ...
    def init_AutoCon(self, args):
        ...
        if self.args.model == 'STEncoderM':  # For multivariate forecasting
            acf_values = np.stack(acf_values, axis=0)
            loss = STEncoderM(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        else:
            acf_values = np.stack(acf_values, axis=0).mean(axis=0)
            loss = STEncoderS(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (timeindex, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)#mask的过程：将预测值盖住
                # encoder - decoder

                if self.args.STEncoder:
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (timeindex, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                batch_x = batch_x.float().to(self.device)
                ###########
                numpy_tensor = batch_x[0,:,:].cpu().detach().numpy()
                df = pd.DataFrame(numpy_tensor)
                df.to_csv('batch_x.csv', index=False)
                ...
                loss = MSE_loss.mean() + self.args.STEncoder_lambda * autocon_loss.mean()
            ...
            print(f"Epoch: {epoch + 1}, Steps: {train_steps} "
                  ...
                  f"AutoCon Loss:{train_log['AutoCon_loss'].mean():.4f} x Lambda({self.args.STEncoder_lambda})), "
