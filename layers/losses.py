...
class STEncoderS(nn.Module):
    ...
    def local_contrastive_loss(self, features, labels):
        ...
        acf = self.acf_values.to(features.get_device())
        local_distmap = acf[local_distmap.abs().long()].float()

        neg_mask = torch.scatter(
            torch.ones_like(local_distmap).to(features.get_device()),
            2,
            torch.arange(T).reshape(1, -1, 1).repeat(B,1,1).to(features.get_device()),#B T 1
            0
        )
    ...
    def avg_global_contrastive_loss(self, features, labels):
        ...
        acf = self.acf_values.to(device)
        global_distmap = acf[global_distmap.abs().long()].float()
        ...
        exp_global_logits = torch.exp(global_logits).to(device) * neg_mask
    ...
    def forward(self, features, labels=None):
        ...
        selected_features = torch.gather(features, 1, feature_idxs.unsqueeze(-1).repeat(1, 1, D))#gather是在指定的维度上收集数据
...
class STEncoderM(nn.Module):  # STEncoder version for Multivariate
    def __init__(self, batch_size, seq_len, acf_values, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07):
        super(STEncoderM, self).__init__()
        self.batch_size = batch_size
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
...
if __name__ == '__main__':
    ...
    supcreg = STEncoderS(acf_values, contrast_mode='all')#.cuda()
